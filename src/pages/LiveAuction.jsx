...
const LiveAuction = () => {
	...
	const setupSubscriptions = () => {
		...
		const auctionSub = supabase
			...
			.on(
				"postgres_changes",
				{ event: "*", schema: "public", table: "auction_state", filter: `tournament_id=eq.${tournamentId}` },
				fetchAuctionState
			)
			.subscribe();
		...
		>
			...
			<main className="p-4 sm:p-6 space-y-4 max-w-5xl mx-auto">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
					<div className={`${light ? "bg-white border-slate-200" : "bg-card-dark border-[#283539]"} rounded-2xl border p-4 shadow-sm`}>
					...
					<div className={`${light ? "bg-white border-slate-200" : "bg-card-dark border-[#283539]"} rounded-2xl border p-4 shadow-sm flex flex-col gap-2`}> 
						...
						<div className="flex items-center justify-between text-sm">
							<span className="text-slate-600 dark:text-text-secondary">Total purse</span>
							<span className="font-bold">{formatCurrency(totalPurse)}</span>
						</div>
						<div className="flex items-center justify-between text-sm">
							<span className="text-slate-600 dark:text-text-secondary">Remaining purse</span>
							<span className="font-bold">{formatCurrency(remainingPurse)}</span>
						</div>
						<div className="w-full h-2 rounded-full bg-slate-200 dark:bg-[#1c2e35] overflow-hidden">
							<div
								className="h-full bg-sky-500"
								style={{ width: `${totalPurse ? Math.max(0, Math.min(100, (remainingPurse / totalPurse) * 100)) : 0}%` }}
							></div>
						</div>
					</div>
				</div>
				...
				<div className={`${light ? "bg-white border-slate-200" : "bg-card-dark border-[#283539]"} rounded-2xl border p-4 shadow-sm`}> 
					<div className="flex items-center justify-between mb-3">
						<div>
							...
							<h2 className="text-xl font-black">{currentPlayer?.name || "Waiting for next player"}</h2>
						</div>
					...
					{currentPlayer ? (
						<div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
									...
									<div className="flex items-center gap-2 text-sm">
										<span
											className={`px-2 py-1 rounded-full border text-xs font-semibold ${
												roleStyles[currentPlayer.role] || "bg-slate-100 text-slate-700 border-slate-200"
											}`}
										>
							...
							<div className="p-3 rounded-xl bg-slate-50 border border-slate-200 dark:bg-[#16262c] dark:border-[#283539]">
								...
								<p className="text-lg font-bold">
									{highestBidder ? formatCurrency(highestBidder.remaining_purse || 0) : "—"}
								</p>
							</div>
						</div>
					) : (
						<div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-600 dark:bg-[#16262c] dark:border-[#283539] dark:text-text-secondary">
							The auction manager will start the next player soon. Keep this page open to watch live bids.
						</div>
					)}
				</div>

				{/* Player list */}
				<div className={`${light ? "bg-white border-slate-200" : "bg-card-dark border-[#283539]"} rounded-2xl border p-4 shadow-sm space-y-3`}>
					<div className="flex items-center justify-between flex-wrap gap-2">
						<div>
							<p className="text-xs uppercase tracking-[0.14em] text-slate-500 dark:text-text-secondary font-semibold">Players</p>
					...
					{players.length === 0 ? (
						<p className="text-sm text-slate-500 dark:text-text-secondary">Players will appear here once added.</p>
					) : (
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
							{sortedPlayers.map((player) => {
								...
								return (
									<div
										key={player.id}
										className={`${light ? "bg-slate-50 border-slate-200" : "bg-[#16262c] border-[#283539]"} border rounded-xl p-3 flex items-start gap-3`}
									>
										<div className="size-10 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden dark:bg-[#1c2e35]">
											{player.photo_url ? (
												...
												<span className="material-symbols-outlined text-slate-500 dark:text-text-secondary">person</span>
											)}
										</div>
										<div className="flex-1 min-w-0 space-y-1">
											<div className="flex items-center justify-between gap-2">
												<p className="font-semibold truncate">{player.name}</p>
												<span className={`px-2 py-0.5 rounded-full text-[11px] font-bold border ${sold ? "bg-green-100 text-green-700 border-green-200 dark:bg-green-500/10 dark:text-green-300 dark:border-green-500/30" : unsold ? "bg-amber-100 text-amber-700 border-amber-200 dark:bg-yellow-500/10 dark:text-yellow-200 dark:border-yellow-500/30" : "bg-blue-100 text-blue-700 border-blue-200 dark:bg-sky-500/10 dark:text-sky-200 dark:border-sky-500/30"}`}>
													{sold ? "Sold" : unsold ? "Unsold" : "Available"}
												</span>
											</div>
											<div className="flex items-center gap-2 text-xs">
												<span className={`px-2 py-0.5 rounded-full border ${roleStyles[player.role] || "bg-slate-100 text-slate-700 border-slate-200"}`}>
													{player.role?.replace("-", " ")}
												</span>
												<span className="text-slate-500 dark:text-text-secondary">
													{sold
														? `Sold for ${formatShortCurrency(player.sold_price || player.base_price || 0)}`
														: `Base ${formatShortCurrency(player.base_price || 0)}`}
												</span>
											</div>
											{sold && (
												<p className="text-xs text-slate-500 dark:text-text-secondary">Bought by {team?.name || "Team"}</p>
											)}
										</div>
									</div>
								);
				...
				<div className={`${light ? "bg-white border-slate-200" : "bg-card-dark border-[#283539]"} rounded-2xl border p-4 shadow-sm space-y-3`}> 
					<div className="flex items-center justify-between">
						...
						<p className="text-xs text-slate-500 dark:text-text-secondary">Tap a team to see its squad</p>
					</div>
					<div className="space-y-3">
						{teams.length === 0 && (
							<p className="text-sm text-slate-500 dark:text-text-secondary">No teams yet.</p>
						)}
						{teams.map((team) => {
							const filled = team.total_purse
								? Math.max(0, Math.min(100, ((team.total_purse - (team.remaining_purse || 0)) / team.total_purse) * 100))
								: 0;
							return (
								<details
									key={team.id}
									className={`${light ? "bg-slate-50 border-slate-200" : "bg-[#16262c] border-[#283539]"} border rounded-xl p-3`}
								>
									<summary className="flex items-center justify-between gap-3 cursor-pointer">
										<div className="flex items-center gap-3 min-w-0">
											<div
												...
												style={{ backgroundColor: `${team.color || "#0db9f2"}1a`, color: team.color || "#0db9f2" }}
											>
												{team.logo_url ? (
													<img
														src={team.logo_url}
											...
											<div className="min-w-0">
												...
												<p className="text-xs text-slate-500 dark:text-text-secondary">
													Remaining {formatShortCurrency(team.remaining_purse || 0)} / {formatShortCurrency(team.total_purse || 0)}
												</p>
											</div>
										</div>
										<div className="text-right">
											<p className="text-xs text-slate-500 dark:text-text-secondary">Spent</p>
											<p className="font-bold">{formatShortCurrency((team.total_purse || 0) - (team.remaining_purse || 0))}</p>
										</div>
									</summary>
...
const SquadList = ({ players, teamId }) => {
	const squad = players.filter((p) => p.team_id === teamId && p.status === "sold");
	if (squad.length === 0) {
		return (
			<p className="text-sm text-slate-500 dark:text-text-secondary">No players bought yet.</p>
		);
	}
			...
			{squad.map((player) => (
				...
				>
					...
					<div className="flex-1 min-w-0">
						...
						<div className="flex items-center gap-2 text-xs">
							<span
								className={`px-2 py-0.5 rounded-full border ${roleStyles[player.role] || "bg-slate-100 text-slate-700 border-slate-200"}`}
							>
								{player.role?.replace("-", " ")}
							</span>
							<span className="text-slate-500 dark:text-text-secondary">
